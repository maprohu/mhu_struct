import 'package:fast_immutable_collections/fast_immutable_collections.dart';
import 'package:isar/isar.dart';
import 'package:mhu_dart_commons/commons.dart';
import 'package:mhu_dart_commons/isar.dart';
import 'package:mhu_dart_pbschema/mhu_dart_pbschema.dart';
import 'package:mhu_dart_pbschema_pbschema/proto.dart';
import 'package:mhu_struct/proto.dart';

import 'generated/mhu_struct.pbschema.dart';
import 'mhu_struct.dart' as $lib;
// part 'mhu_struct.g.has.dart';
part 'mhu_struct.g.dart';
// part 'mhu_struct.freezed.dart';


part 'isar.dart';
part 'registry.dart';
part 'builtin.dart';
part 'builtin/path.dart';
